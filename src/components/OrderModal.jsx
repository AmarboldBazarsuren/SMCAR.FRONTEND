...
const CHANNELS = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
        <path d="M12 0C5.373 0 0 4.974 0 11.111c0 3.498 1.744 6.614 4.469 8.652V24l4.088-2.242c1.092.301 2.246.464 3.443.464 6.627 0 12-4.975 12-11.111C24 4.974 18.627 0 12 0zm1.191 14.963l-3.055-3.26-5.963 3.26L10.732 8.1l3.131 3.26L19.752 8.1l-6.561 6.863z"/>
      </svg>
    ),
    color: 'bg-[#0084FF]',
...
export default function OrderModal({ isOpen, onClose, vehicleId }) {
  ...
  const handleCopyId = () => {
    if (vehicleId) {
      navigator.clipboard.writeText(String(vehicleId))
        .then(() => alert('Машины дугаар хуулагдлаа!'))
        .catch(() => {})
    }
  }
          ...
          <div className="grid grid-cols-2 gap-3">
            {CHANNELS.map(ch => (
              <a
                ...
                className={`flex flex-col items-center gap-2 ${ch.color} ${ch.hoverColor} text-white rounded-xl py-4 px-3 transition-all duration-200 hover:scale-105 active:scale-95`}
              >
                {ch.icon}
